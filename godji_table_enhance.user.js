...
(function () {
    ...
    // ── Попап подтверждения (по структуре ERP modal) ─────────────────────────
    function showConfirm(title, text, confirmLabel, confirmColor, onConfirm) {
        ...
        cancelBtn.style.cssText = '--button-bg:var(--mantine-color-default);--button-hover:var(--mantine-color-default-hover);--button-color:var(--mantine-color-default-color);--button-bd:calc(0.0625rem * var(--mantine-scale)) solid var(--mantine-color-default-border);';
        ...
        okBtn.style.cssText = '--button-bg:'+confirmColor+';--button-hover:'+confirmColor+';--button-color:var(--mantine-color-white);--button-bd:calc(0.0625rem * var(--mantine-scale)) solid transparent;filter:none;';
        ...
        flex.appendChild(cancelBtn); flex.appendChild(okBtn);
    ...
    // ── Перехват кнопок меню ──────────────────────────────────────────────────
    // Кнопки «Завершить сеанс» и «Выключить ПК» больше не имеют confirm от ERP.
    // Перехватываем клик в фазе capture до того как React его обработает.

    var INTERCEPT_LABELS = {
        'Завершить сеанс': {
            title:   'Завершить сеанс?',
            text:    'Сеанс будет завершён принудительно. Это действие нельзя отменить.',
            label:   'Завершить',
            color:   'var(--mantine-color-red-filled)'
        },
        'Выключить ПК': {
            title:   'Выключить ПК?',
            text:    'Компьютер будет выключен. Убедитесь, что клиент завершил работу.',
            label:   'Выключить',
            color:   '#b71c1c'
        },
        'Перезагрузить ПК': {
            title:   'Перезагрузить ПК?',
            text:    'Компьютер будет перезагружен.',
            label:   'Перезагрузить',
            color:   '#e65100'
        }
    };

    // Используем capture на document — срабатывает раньше React
    document.addEventListener('click', function(e) {
        // Ищем menuitem вверх по дереву от цели клика
        var el = e.target;
        var menuItem = null;
        for (var i = 0; i < 6; i++) {
            if (!el) break;
            if (el.getAttribute && el.getAttribute('role') === 'menuitem') { menuItem = el; break; }
            el = el.parentElement;
        }
        if (!menuItem) return;

        // Проверяем что это пункт меню ERP (не наш)
        if (menuItem.hasAttribute('data-godji-confirm-intercepted')) return;

        var label = menuItem.textContent ? menuItem.textContent.trim() : '';
        var cfg = INTERCEPT_LABELS[label];
        if (!cfg) return;

        // Получаем название ПК из строки таблицы (для текста попапа)
        var pcName = '';
        try {
            var row = document.querySelector('tr.mantine-Table-tr[data-selected="true"], tr.mantine-Table-tr:hover');
            if (row) {
                var pcCell = row.querySelector('td[style*="col-deviceName-size"]');
                if (pcCell) pcName = pcCell.textContent.trim();
            }
        } catch(err) {}

        var textWithPc = cfg.text + (pcName ? '<br><br><strong>ПК: ' + pcName + '</strong>' : '');

        // Блокируем оригинальный клик
        e.stopImmediatePropagation();
        e.preventDefault();

        // Сохраняем ссылку на кнопку чтобы кликнуть после подтверждения
        var targetBtn = menuItem;
        showConfirm(cfg.title, textWithPc, cfg.label, cfg.color, function() {
            // Помечаем чтобы не перехватить повторно
            targetBtn.setAttribute('data-godji-confirm-intercepted', '1');
            targetBtn.click();
            // Сбрасываем флаг после выполнения
            setTimeout(function(){ targetBtn.removeAttribute('data-godji-confirm-intercepted'); }, 500);
        });
    }, true); // capture = true — раньше React
