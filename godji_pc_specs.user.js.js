...
/* ═══════════════════════════════════════════════════════
   КОНФИГУРАЦИЯ И ДАННЫЕ ПО УМОЛЧАНИЮ (Клуб Гоголя)
   ═══════════════════════════════════════════════════════ */
const STORAGE_KEY = 'godji_pc_specs_v1';
...
function load() {
  ...
  if (!_data) {
    _data = { structure: JSON.parse(JSON.stringify(DEFAULT_STRUCTURE)), specs: JSON.parse(JSON.stringify(DEFAULT_SPECS)) };
    save();
  }
  if (!_data.structure) _data.structure = JSON.parse(JSON.stringify(DEFAULT_STRUCTURE));
...
function getDeviations(zoneId, roomId) {
  ...
  Object.keys(roomSpecs).forEach(function(key) {
    ...
    if (!zoneVal || zoneVal !== roomSpecs[key].v) {
      diffs.room.push({ key: key, val: roomSpecs[key].v, zoneVal: zoneVal });
    }
  });
...
function createModal() {
  ...
  _overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:99998;display:none;align-items:center;justify-content:center;padding:16px;';
...
function buildZone(zone) {
  ...
  body.style.cssText = 'padding:10px;display:block;background:#fff;';
  ...
  if(_isEdit) {
    var specEditor = buildSpecEditor('zone_'+zone.id, true);
    body.appendChild(specEditor);
  }
  return wrap;
}
...
function buildRoom(zoneId, roomId) {
  ...
  body.style.cssText = 'padding:8px 0 10px;display:block;';
...
function mkInfoBtn(specKey, title, parentZoneId) {
  ...
  btn.addEventListener('click', function(e){
    e.stopPropagation();
    showInfoPopup(btn, specKey, title, parentZoneId);
  });
  return btn;
}
...
function mkDragHandle(type, id) {
  ...
  el.addEventListener('dragstart', function(e){
    _dragData = { type: type, id: id };
    e.dataTransfer.effectAllowed = 'move';
    e.target.style.opacity = '0.4';
  });
  el.addEventListener('dragend', function(e){ e.target.style.opacity='1'; _dragData=null; });
...
function handleDrop(targetType, targetId, dragType, dragId) {
  if(dragType==='room' && targetType==='room') {
    var srcZ = null, tgtZ = null;
...
function buildSpecEditor(key, isZone) {
  ...
  addBtn.addEventListener('mouseenter', function(){addBtn.style.background='#e9ecef';});
