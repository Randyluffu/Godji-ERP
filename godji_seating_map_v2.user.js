// ==UserScript==
// @name         Годжи — Карта посадки v2.3 (FIXED)
...
(function(){
'use strict';
// Работаем только на главной (карте)
...
function buildSVG(isTV){
    var ns='http://www.w3.org/2000/svg';
    var svg=document.createElementNS(ns,'svg');
    
    // Фиксированный размер холста для совпадения с POS
    var cw = 1600, ch = 1200;
    svg.setAttribute('viewBox', '0 0 ' + cw + ' ' + ch);
    svg.style.cssText='position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:0;';

    // Коэффициенты трансформации (Admin)
    var sx = 1.76, sy = 2.15, tx = 100, ty = 60;
    
    // Если ТВ-панель, используем другие настройки подгона
    if(isTV) { sx = 1.6; sy = 1.6; tx = 150; ty = 50; }
    ...
    function poly(pts,fill,stroke){
        ...
        if(stroke){el.setAttribute('stroke',stroke);el.setAttribute('stroke-width',1.2);}
        return el;
    }
    g.appendChild(poly(FLOOR,'rgba(195,210,235,0.4)','rgba(155,175,215,0.3)'));
    ROOMS.forEach(function(r){
        g.appendChild(poly(SHAPES[r.id],'rgba(255,255,255,0.85)','rgba(170,188,220,0.5)'));
    });
    svg.appendChild(g);
    return svg;
}
...
function injectAdmin(){
    ...
    if(!layer || layer._gmDone) return;
    layer._gmDone = true;
    
    // Принудительно задаем размер слоя, чтобы он не схлопывался
    layer.style.width = '1600px';
    layer.style.height = '1200px';
    layer.style.position = 'relative';
    
    layer.insertBefore(buildSVG(false), layer.firstChild);
    repositionCards(layer);
    
    new MutationObserver(() => repositionCards(layer)).observe(layer, {childList:true, subtree:true});
}

function injectTV(){
    var tv=document.querySelector('.TVMapCanvas_devicesLayer__4NfZg');
    if(!tv || tv._gmDone) return;
    tv._gmDone = true;
    
    // Для ТВ-карты растягиваем контейнер
    tv.style.width = '1600px';
    tv.style.height = '1200px';
    tv.style.position = 'relative';
    
    tv.insertBefore(buildSVG(true), tv.firstChild);
}

function init(){
    if(window.location.pathname.includes('/tv/')) injectTV();
    else injectAdmin();
}

// Запуск и слежение за переходами
