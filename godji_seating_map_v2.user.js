...
function buildSVG(isTV){
    ...
    // Коэффициенты для админки и ТВ разные, чтобы всё совпало
    let sx = isTV ? 1.6 : 1.76, sy = isTV ? 1.6 : 2.15, tx = isTV ? 150 : 100, ty = isTV ? 50 : 60;
    ...
    Object.values(SHAPES).forEach(pts => {
        ...
        p.setAttribute('stroke','rgba(170,188,220,0.5)');
...
function inject(){
    ...
    layer._done = true;
    let isTV = window.location.pathname.includes('/tv/');
    
    layer.style.width = '1600px';
    layer.style.height = '1200px';
    layer.style.position = 'relative';
    layer.insertBefore(buildSVG(isTV), layer.firstChild);
